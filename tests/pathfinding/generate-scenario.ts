 ...
 *
 * Usage:
 *   npx tsx tests/pathfinding/generate-scenario.ts <map-name> [--force]
 *   npx tsx tests/pathfinding/generate-scenario.ts --all [--force]
 *
 * Examples:
 *   npx tsx tests/pathfinding/generate-scenario.ts iceland
 *   npx tsx tests/pathfinding/generate-scenario.ts giantworldmap --force
...
async function main() {
  ...
  if (args.includes("--all")) {
    ...
    if (result === 'created') {
      ...
      console.log(`You can now run: npx tsx tests/pathfinding/benchmark.ts synthetic/${mapName}`);
    } else {
      process.exit(1);
    }
  } else {
...
function generateScenarioContent(params: TemplateParams): string {
  const { mapName, ports, routes, performanceIterations = 10 } = params;

  let content = ``;

  content += `import { dirname, join } from "path";\n`;
  content += `import { fileURLToPath } from "url";\n`;
  content += `import { setupFromPath } from "../../utils";\n\n`;
  content += `export const PERFORMANCE_ITERATIONS = ${performanceIterations}\n\n`;
  ...
  // Generate setupGame function
  content += `export async function setupGame() {\n`;
  content += `  const currentFile = fileURLToPath(import.meta.url);\n`;
  content += `  const scenariosDir = dirname(currentFile);\n`;
  content += `  const projectRoot = join(scenariosDir, "../../../..");\n`;
  content += `  const mapsDirectory = join(projectRoot, "resources/maps");\n`;
  content += `  return await setupFromPath(mapsDirectory, "${mapName}");\n`;
  content += `}\n`;
