...
interface PathfindingOptions {
  ...
  includeNavSat?: boolean;
}

interface NavSatResult {
...
const pfMiniCache = new Map<string, PathFinderMiniAdapter>();

/**
 * Get or create PathFinder.Mini adapter for a map
 */
function getPfMiniAdapter(mapName: string, game: any): PathFinderMiniAdapter {
  if (!pfMiniCache.has(mapName)) {
    const adapter = new PathFinderMiniAdapter(game);
...
export async function computePath(
...
): Promise<NavSatResult> {
  const { game, navSat: navSatAdapter } = await loadMap(mapName);
  ...
  // Compute NavigationSatellite path
  const navSatPath = navSatAdapter.findPath(fromRef, toRef, true);
  const path = pathToCoords(navSatPath, game);
  ...
  if (navSatAdapter.debugInfo) {
    // Convert gatewayPath (TileRefs on miniMap) to full map coordinates
    if (navSatAdapter.debugInfo.gatewayPath) {
      gateways = navSatAdapter.debugInfo.gatewayPath.map((tile: TileRef) => {
    ...
    if (navSatAdapter.debugInfo.initialPath) {
      initialPath = navSatAdapter.debugInfo.initialPath.map((tile: TileRef) =>
        [game.x(tile), game.y(tile)] as [number, number]
      );
    }

    timings = navSatAdapter.debugInfo.timings || {};
  }
