...
export async function getMapMetadata(mapName: string) {
  const { game, navSat } = await loadMap(mapName);
  ...
  const gatewaysArray = Array.from(navSatGraph.gateways.values());
  ...
  const allEdges: any[] = [];
  for (const edgeArray of navSatGraph.edges.values()) {
  ...
  const sectorSize = navSatGraph.sectorSize;
