...
const state = {
    ...
    endPoint: null,
    navSatPath: null,
    navSatResult: null, // Store full NavSat result including timing
    pfMiniPath: null,
    ...
    isNavSatLoading: false, // Separate loading state for NavSat
...
function initializeControls() {
    ...
    // Refresh NavSat button
    document.getElementById('refreshNavSat').addEventListener('click', (e) => {
...
function initializeTimingsPanel() {
    ...
    updateTimingsPanel({ navSat: null, pfMini: null });
    updatePfMiniButton();
}

// Handle map clicks for point selection
function handleMapClick(e) {
    if (!state.currentMap || state.isMapLoading || state.isNavSatLoading || state.isPfMiniLoading) return;
...
function clearPoints() {
    ...
    state.endPoint = null;
    state.navSatPath = null;
    state.navSatResult = null;
    state.pfMiniPath = null;
    state.pfMiniResult = null;
...
async function requestPathfinding(from, to) {
    ...
    try {
        ...
        // Update state
        state.navSatPath = result.path;
        state.navSatResult = result; // Store full result for later use
        // Don't reset pfMiniPath - preserve it across NavSat refreshes
        state.debugInfo = {
        ...
        updatePathInfo({ navSat: result, pfMini: state.pfMiniResult });
        renderInteractive();
...
// Request PF.Mini computation only (without re-computing NavSat)
async function requestPfMiniOnly(from, to) {
    ...
    try {
        ...
        // Update only PF.Mini path (preserve existing NavSat path and debug info)
        state.pfMiniPath = result.path;
        state.pfMiniResult = result; // Store full result

        // Update UI (preserve existing NavSat result)
        updatePathInfo({ navSat: state.navSatResult, pfMini: result });
        renderInteractive();
...
function updatePfMiniButton() {
    ...
    } else if (state.startPoint && state.endPoint && state.navSatPath) {
        // Show and enable button when points are set and NavSat path exists
        requestSection.style.display = 'block';
...
function updateTimingsPanel(result) {
    const navSat = result.navSat;

    // Show NavSat time and path length (or 0.00 in light gray if no data)
    const navSatTimeEl = document.getElementById('navSatTime');
    if (navSat && navSat.time > 0) {
        navSatTimeEl.textContent = `${navSat.time.toFixed(2)}ms`;
        navSatTimeEl.classList.remove('faded');
    } else {
        navSatTimeEl.textContent = '0.00ms';
        navSatTimeEl.classList.add('faded');
    }

    const navSatTilesEl = document.getElementById('navSatTiles');
    if (navSat && navSat.length > 0) {
        navSatTilesEl.textContent = `- ${navSat.length} tiles`;
    } else {
        navSatTilesEl.textContent = '';
    }

    // Show timing breakdown - always visible with gray dashes when no data
    const timings = (navSat && navSat.timings) ? navSat.timings : {};
    ...
    if (result.pfMini && result.pfMini.time > 0) {
        ...
        if (navSat && navSat.time > 0) {
            const speedup = result.pfMini.time / navSat.time;
...
function renderOverlay(scale) {
    ...
    if (showSectorGrid && state.graphDebug.sectorSize) {
        const sectorSize = state.graphDebug.sectorSize;
...
function renderInteractive() {
    ...
    // Draw NavSat path
    if (state.navSatPath && state.navSatPath.length > 0) {
        ...
        for (let i = 0; i < state.navSatPath.length; i++) {
            const [x, y] = state.navSatPath[i];
            const screen = mapToScreen(x + 0.5, y + 0.5);
            if (i === 0) {
                interactiveCtx.moveTo(screen.x, screen.y);
            } else {
