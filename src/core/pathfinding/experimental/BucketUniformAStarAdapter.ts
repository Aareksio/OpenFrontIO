// Adapter for BucketUniformAStar (GraphAdapter + uniform cost)
...
import { BucketUniformAStar } from "./BucketUniformAStar";
import { fixExtremes, upscalePath } from "./PathUpscaler";

class GameMapGraphAdapter implements GraphAdapter<number> {
  constructor(private gameMap: GameMap, private width: number) {}
  neighbors(node: number): number[] { return this.gameMap.neighbors(node as TileRef) as number[]; }
  cost(node: number): number { return 1; } // Uniform cost
  position(node: number): { x: number; y: number } {
    return { x: this.gameMap.x(node as TileRef), y: this.gameMap.y(node as TileRef) };
  }
  isTraversable(_from: number, to: number): boolean { return this.gameMap.isWater(to as TileRef); }
}

export class BucketUniformAStarAdapter implements PathFinder {
  private game: Game;
  private aStar: BucketUniformAStar;

  constructor(game: Game, options?: { iterations?: number; maxTries?: number }) {
    this.game = game;
    const miniMap = game.miniMap();
    const width = miniMap.width();
    const graphAdapter = new GameMapGraphAdapter(miniMap, width);
    const numNodes = width * miniMap.height();

    this.aStar = new BucketUniformAStar(
      graphAdapter, numNodes, width,
