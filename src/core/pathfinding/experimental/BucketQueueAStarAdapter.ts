...
import { BucketQueueAStar } from "./BucketQueueAStar";
import { fixExtremes, upscalePath } from "./PathUpscaler";

export interface BucketQueueAStarOptions {
  iterations?: number;
  maxTries?: number;
}
...
export class BucketQueueAStarAdapter implements PathFinder {
  private game: Game;
  private graphAdapter: GameMapGraphAdapter;
  private aStar: BucketQueueAStar;

  constructor(game: Game, options?: BucketQueueAStarOptions) {
    this.game = game;
    ...
    this.aStar = new BucketQueueAStar(
      this.graphAdapter,
      numNodes,
      width,
